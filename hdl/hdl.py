...
class HDL(object):
    ...
    def __init__(self, model_sequence, datasource, **kargs):
        ...
        for layer in range(len(self.model_sequence)):
            ...
            sched_list = [{'iterations':200000},
                          {'iterations':200000,'change_target':.5},
                          {'iterations':100000,'change_target':.5}]
    ...
    def learn(self,layer_start=0):
        ...
        for mind, m in enumerate(self.model_sequence[layer_start:]):
            if not mind:
                ...
                l = learners.SGD_layer(model=m,datasource=self.datasource,display_every=20000,batchsize=self.batchsize,model_sequence=self.model_sequence[:mind],layer_params=self.layer_params)
